...
var isdosINI: int = 60 #Time in seconds to be considered attack, must be greater than 10 
...
func verify(ip) -> Dictionary:
	...
	if (connectionsINI <= 0 or isdosINI <= 0) and firewallINI == false and not bannedListINI and IPDataCheckINI != true:
		print("WARNING, all protections are disabled, it is recommended to set at least $connectionsINI to 5 and $isdosINI to 100")
		return dummy
	elif attack(ip):
		bannedIP.append(ip)
		dummy['result'] = true
